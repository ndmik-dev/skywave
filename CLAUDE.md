...
**Збудовано все з плану, крім ShazamKit.** Сесії 0–6 закриті за один тиждень:
каталог і пʼять адаптерів, ICY-метадані, Now Playing і медіаклавіші, хоткей,
автозапуск, стійкість (сон, мережа, застій, сторож), «Моменти» через друге
зʼєднання, таймер сну, сповіщення про ефіри й відключення, логотипи станцій,
пошук і клавіатурна навігація, мертві станції. 24 коміти, останній 1 вересня.

**Що не зроблено — і рішення:**

- **Не встановлений і не використовується.** Це єдиний пункт, що має значення.
  `./Scripts/bundle.sh --install` — і слухати місяць. Жодних змін у панелі в цей час.
- **ShazamKit — не робити.** Тап віддає звук на 5 станціях із 20, для решти
  потрібне друге зʼєднання, яке вже є для «Моментів». Але ~40% ефіру NTS немає
  навіть на Spotify — Shazam мовчатиме там, де потрібен найбільше. Ціна $99/рік
  за підпис заради фічі, що працює через раз. Знято з плану.
- **«Моменти» лишаються** — вони на другому зʼєднанні, тож покриття повне.
- **Дві правки панелі — після місяця, не до:**
  перемикач «Ефіри / Моменти» зроблений на всю ширину, а в панелі на 300px
  кожен піксель висоти дорожчий за ширину — сегмент у шапці поруч із назвою
  станції робить панель на рядок нижчою. І довідка `?` з хоткеями, як у Sofar:
  тут їх не менше, а підказки немає.
- **Чесне питання після місяця:** скільки з двадцяти станцій справді вмикалось?
  Якщо три — тоді мультистанційність не була нішею, і Safari «Add to Dock» для
  NTS покрив би все. Відповідь на це важливіша за будь-яку нову функцію.
